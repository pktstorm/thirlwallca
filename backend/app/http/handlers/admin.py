...
from app.domain.models import AuditLog, ErrorLog, OnboardToken, SignupCode, SignupRequest, User
...
async def list_error_logs(
    ...
    """List error logs, optionally filtered by resolved status."""
    _require_admin(current_user)
    ...
    return [
        ErrorLogResponse(
            id=str(e.id), method=e.method, path=e.path,
            status_code=e.status_code, error_type=e.error_type,
            error_message=e.error_message, traceback=e.traceback,
            user_agent=e.user_agent, ip_address=e.ip_address,
            request_body=e.request_body, resolved=e.resolved,
...
async def get_error_stats(
    ...
    """Get error log summary stats."""
    _require_admin(current_user)
    total = await db.execute(select(func.count(ErrorLog.id)))
    ...
    return {
        ...
        "last_24h": recent_24h.scalar() or 0,
    }


@router.put("/error-logs/{error_id}/resolve")
async def toggle_error_resolved(
    error_id: str,
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    """Toggle an error log's resolved status."""
    _require_admin(current_user)
    ...
    await db.flush()
    return {"resolved": error.resolved}


@router.delete("/error-logs/{error_id}", status_code=204)
