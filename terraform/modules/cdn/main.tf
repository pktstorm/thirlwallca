# CloudFront Function to strip /media prefix for S3 origin
resource "aws_cloudfront_function" "media_uri_rewrite" {
  name    = "${var.project_name}-media-uri-rewrite"
...
resource "aws_cloudfront_distribution" "main" {
  ...
  ordered_cache_behavior {
    ...
    forwarded_values {
      query_string = true
      headers      = ["Authorization", "Content-Type", "Host", "Origin", "X-Forwarded-Proto"]
      cookies {
        forward = "all"
      }
    }
  ...
  # Media behavior -> S3 media bucket via OAC
  ordered_cache_behavior {
    path_pattern     = "/media/*"
    allowed_methods  = ["GET", "HEAD", "OPTIONS"]
    cached_methods   = ["GET", "HEAD"]
...
# Allow CloudFront OAC to read from media bucket
